...
public class ManCityHomeViewController: UIViewController {
    ...
    func setUpNavigationBar(isLightContent: Bool = true) {
        ...
        if let backImage = UIImage(named: AppCommonMethods.languageIsArabic() ? "back_icon_ar" : "back_icon", in: .module, compatibleWith: .current) {
            btnBack.setImage(backImage, for: .normal)
        } else {
            btnBack.setImage(UIImage(systemName: "return"), for: .normal)
        }
